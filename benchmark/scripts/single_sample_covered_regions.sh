...
awk '$4 == 2 && $1 != "chrX" && $1 != "chrY"' $TMPDIR/hq_diploid_cov.bed | cut -f1-3 > $TMPDIR/${outname}_hq_cov.bed
# chrY requires 1x
awk '$4 == 1 && $1 == "chrY"' $TMPDIR/hq_diploid_cov.bed | cut -f1-3 >> $TMPDIR/${outname}_hq_cov.bed
...
bedtools subtract -a $TMPDIR/hq_diploid_cov.chrX.bed -b $par_reg \
    | awk '$4 == 1' \
    | cut -f1-3 >> $TMPDIR/${outname}_hq_cov.bed
...
bedtools intersect -u -f 1 -a $trr -b $TMPDIR/${outname}_hq_cov.bed > $TMPDIR/${outname}_covered_spans.bed
